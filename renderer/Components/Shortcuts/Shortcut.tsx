...
function Shortcut(props: Props) {
    ...
    const handleMouseDown = (e) => {
        ...
        if (button === 0) {
            ...
            const elementDrag = (e) => {
                ...
                if (shortcuts.short_by_grid) {
                    ...
                    if (index == shortcuts.items.length) {
                        ...
                        dTop =
                            shortcuts.items[index - 1].y +
                            getSizeOfShortcutItem(shortcuts.size).h -
                            10;
                    } else if (
                        index != 0 &&
                        index % sortShortcuts(shortcuts).row == 0 &&
                        top > shortcuts.items[0].y
                    ) {
                        dLeft = shortcuts.items[index - 1].x;
                        dTop =
                            shortcuts.items[index - 1].y +
                ...
                if (shortcuts.short_by_grid)
                    document.getElementById(data.id + "-clone").classList.remove("hidden");
            };
            const getIndex = (x: number, y: number) => {
                ...
                if (x <= firstItem.x && y < firstItem.y + 10) return 0;
                if (
                    x <= firstItem.x + getSizeOfShortcutItem(firstItem.size).w / 2 &&
                    y < firstItem.y
                )
                    return 0;
                // ?[bg-info] cuối
                if (x >= lastItem.x && y >= lastItem.y) return shortcuts.items.length;
                if (x >= lastItem.x && y > lastItem.y + 10) return shortcuts.items.length;
                if (x >= lastItem.x + 10 && y > lastItem.y) return shortcuts.items.length;
                ...
                if (y > shortcuts.items[row - 1].y) {
                    ...
                    while (shortcuts.items[row * i - 1]) {
                        if (
                            x <
                            shortcuts.items[row * i - 1].x +
                                getSizeOfShortcutItem(shortcuts.size).w / 2
                        )
                            return row * i > shortcuts.items.length - 1
                ...
                if (y < shortcuts.items[row].y) {
                    ...
                    while (shortcuts.items[row * i]) {
                        if (
                            x <
                            shortcuts.items[row * i].x + getSizeOfShortcutItem(shortcuts.size).h / 2
                        )
                            return row * i;
                        i++;
                    }
            ...
            const closeDragElement = () => {
                ...
                if (shortcuts.short_by_grid) {
                    ...
                    if (
                        indexRef.current == -1 ||
                        shortcuts.items[indexRef.current]?.id == data.id
                    ) {
    ...
    return (
        <>
            ...
            >
                <div className="relative">
                    <div className="icon rounded overflow-hidden">
                        <Image
                            src={data.icon}
                            alt="..."
                            width={SIZE[data.size].value * 0.8}
